...
module.exports = {
	...
	data: new SlashCommandBuilder()
		.setName("monster")
		.setDescription(
			"Displays the statblock of a SRD licensed Dungeons and Dragons 5e monster statblock."
		)
		.addStringOption((option) => {
			return (option = option
				.setName("monster")
				.setDescription("Enter the name of the monster.")
				.setRequired(true));
		}),
	async execute(interaction, bot) {
		...
		if (!monsterInfo)
			return interaction.reply({
				embeds: [
					embedify(
						"That is not an available monster. Either you spelled it wrong, or that's not a SRD licensed monster. Use the command `monsterlist` to view all available monsters"
					),
				],
			});
